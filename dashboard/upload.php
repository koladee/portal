        ...
        if(isset($_FILES['file'])){
            ...
            if (!preg_match("/([^\w\s\d\-_~,;:\[\]\(\).])|([\.]{2,})/", $_FILES['file']['name'])) {
                if (in_array(strtolower(pathinfo($_FILES['file']['name'], PATHINFO_EXTENSION)), array("jpeg", "jpg", "png"))) {
                    ...
                    $stmt = $db->query('select photo from users where uniqid="'.$_SESSION['uniqid'].'"');
                    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $dat = $rows[0];
    $wr = $dat['photo'];
                    if(($wr !== "im12ages784345/male.jpg" || $wr !== "im12ages784345/female.jpg") && $wr != "" ){
                        unlink("../".$wr);
                    }
                    $db->query('update users set photo="im12ages784345/'.$filename.'" where uniqid="'.$_SESSION['uniqid'].'"');
